...
OUTPUT = REPO_ROOT / "docs" / "AWT_Lab_Terminal_MERN_Submission.docx"
...
def build_rest(doc):
    ...
    add_paragraph(
        doc,
        "All API routes are prefixed with /api. Protected routes expect an Authorization header "
        "of the form Bearer <token>. Responses use JSON with a success flag and a data field "
...
def build_data(doc):
    ...
    add_paragraph(
        doc,
        "MongoDB stores four collections. Relationships use ObjectId references and "
        "populate() where related names are needed in API responses.",
    )
...
def build_ui(doc):
    ...
    shots = [
        ("01-home.png", "Figure 1: Home page with search and filters for lost and found listings."),
        ("02-login.png", "Figure 2: Login page."),
        ("03-register.png", "Figure 3: Registration page for new users."),
        ("04-report-item.png", "Figure 4: Report item form for logged-in users."),
        ("05-my-items.png", "Figure 5: My items page listing the user's own reports."),
        ("06-my-claims.png", "Figure 6: My claims page showing submitted ownership requests."),
        ("07-profile.png", "Figure 7: Profile page for updating account details."),
        ("08-admin-claims.png", "Figure 8: Admin claims review page."),
        ("09-admin-stats.png", "Figure 9: Admin statistics dashboard."),
    ]
    detail = SCREENSHOTS / "03b-item-detail.png"
    if detail.exists():
        shots.insert(3, ("03b-item-detail.png", "Figure 4: Item detail page with comments."))
        for i in range(4, len(shots)):
            fname, cap = shots[i]
            num = i + 1
            cap = cap.split(":", 1)[1].strip()
            shots[i] = (fname, f"Figure {num}: {cap}")
